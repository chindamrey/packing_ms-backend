    ...
    <div class="page">
        ...
        <div class="header">
            <div>
                ...
                <p class="page-subtitle">Authorize and manage priority parking access for special whitelist vehicles.
                </p>
            </div>
        ...
        <div class="stats-row">
            <div class="stat-card">
                ...
                <div class="stat-value">{{ usePackingLogs.total }}</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">IN PARCKING</div>
                <div class="stat-value stat-value--blue">{{ usePackingLogs.allStatusInfo?.data?.enter_car }}</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">EXIT CAR</div>
                <div class="stat-value stat-value--red">{{ usePackingLogs.total -
                    usePackingLogs.allStatusInfo?.data?.enter_car }}</div>
            </div>
        ...
        <div class="table-card">
            ...
            <div class="table-footer">
                <span class="entry-count">
                    Showing {{ usePackingLogs.rangeStart }} to {{ usePackingLogs.rangeEnd }}
                    of {{ usePackingLogs.total }} entries
                </span>

                <div class="pagination d-none">
                    <button class="pg-btn" :disabled="usePackingLogs.currentPage === 1" @click="prevPage">
                        &#8249;
                    </button>

                    <button v-for="p in usePackingLogs.totalPages" :key="p" class="pg-btn"
                        :class="{ 'pg-btn--active': p === usePackingLogs.currentPage }"
                        @click="usePackingLogs.setPage(p)">
                        {{ p }}
                    </button>

                    <button class="pg-btn" :disabled="usePackingLogs.currentPage === usePackingLogs.totalPages"
                        @click="nextPage">
                        &#8250;
                    </button>
                </div>
                <div class="pagination">
                    ...
                    <button class="pg-btn" v-for="(p, index) in pagination" :key="index" :disabled="p === '...'"
                        :class="{ 'plate-badge': p === usePackingLogs.currentPage }" @click="p !== '...' && goToPage(p)">
                        {{ p }}
                    </button>

                    <button class="pg-btn" :disabled="usePackingLogs.currentPage === usePackingLogs.totalPages" @click="nextPage">
                        ›
                    </button>
...
onMounted(async () => {
    usePackingLogs.currentPage = 1;
    usePackingLogs.perPage = 10;
    await usePackingLogs.getRecentActivities();
    await usePackingLogs.getAllStatus();
    // console.log(await usePackingLogs.allStatusInfo);

    //   console.log('recent activities  ', usePackingLogs.recentActivities)
})
...
const goToPage = async (page) => {
    usePackingLogs.currentPage = page
    await usePackingLogs.getRecentActivities()
}

const pagination = computed(() => {

    const total =usePackingLogs.totalPages
    current.value = usePackingLogs.currentPage
    const delta = 1
    console.log(total,current.value);
    
    const range = []
    const rangeWithDots = []

    let left = current.value - delta
    let right = current.value + delta
